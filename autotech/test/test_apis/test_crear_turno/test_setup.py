...
class TestSetUp(APITestCase):
    def setUp(self):
        ...
        self.turno_test_1 = G(Turno_taller, patente = self.patente_cliente, id_turno= 1, taller_id=11, tipo='evaluacion', estado="pendiente", tecnico_id= None, fecha_inicio=date(2023,9,21), hora_inicio=time(10,0,0), fecha_fin=date(2023,9,21), hora_fin=time(12,0,0), papeles_en_regla=True)
        ...
        self.service3 = G(Service, id_service= 400, costo_base = 0.0, costo_total = 0.0, marca="generico", modelo = "generico", frecuencia_km=20000, duracion_total=180)
        
        # la patente esa ya hizo el service de 5k
        self.turno_service = G(Turno_taller, patente = self.patente_cliente, id_turno= 500, tipo='service', frecuencia_km= 5000, estado="terminado", tecnico_id= None, fecha_inicio=date(2023,4,21), hora_inicio=time(10,0,0), fecha_fin=date(2023,4,21), hora_fin=time(12,0,0), papeles_en_regla=True)
        ...
        self.turno_evaluacion = G(Turno_taller, taller_id = 10, patente = self.patente_evaluada, id_turno= 400, tipo='evaluacion', estado="terminado", fecha_inicio=date(2023,4,20), hora_inicio=time(10,0,0))
        self.turno_extraordinario = G(Turno_taller, taller_id = 10, patente = self.patente_cliente, id_turno= 401, tipo='extraordinario', estado="terminado", fecha_inicio=date(2023,4,20), hora_inicio=time(10,0,0))
