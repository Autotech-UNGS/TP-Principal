...
class ModificarTaller(APIView):
    ...
    def put(self, request, id_taller):
        ...
        if turnos_pendientes and taller.estado and not estado_nuevo:
        ...
        # valido que no exista ya un taller creado para esa sucursal
        try:
            taller_sucursal = Taller.objects.get(id_sucursal=id_sucursal_taller)
        except Taller.DoesNotExist:
            return Response({'error': f'No existe un taller para la sucursal {id_sucursal_taller}'}, status=status.HTTP_404_NOT_FOUND)
        
        # Valido que ambas sucursales existan y sean validas
        try:
        ...
        # Valido que el taller sea valido para la nueva sucursal 
        try:
        ...
        # ------------------------------------------------------------------------------------------------------ #
            ...
            actualizar_taller_admin_view = ActualizarTallerAdmin()
            response = actualizar_taller_admin_view.put(request, taller.id_sucursal)
            response_data["actualizacion_sucursal"].append({"code":f"{response.status_code}"})
            response_data["resultado"].append(response.data)
